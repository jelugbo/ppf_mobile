...
import {HelpdeskmodalPage} from '../helpdeskmodal/helpdeskmodal.page';

@Component({
  selector: 'app-helpdesks',
  templateUrl: './helpdesks.page.html',
  styleUrls: ['./helpdesks.page.scss'],
})
export class HelpdesksPage implements OnInit {
  showItems;
  itemList;
  ...
  getItems(){
    ...
    this.apiCall.fetchData('pluck/funds' , 'Type=Help&ExpiryDate=' + exp, false).subscribe(
        data => {
  ...
  itemTapped(item){
    ...
    this.router.navigateByUrl('/menu/helpdesk/' + item.Id);
  }

  requestor(item){
    this.storage.get('USER_ID').then(
        (id) => {
          const putData = {RequestorId: id, Type: 'Help', ItemId: item.Id};
